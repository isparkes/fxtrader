...
Patterns A and C:
  Stop loss:   HA_SL_MIN_PIPS floor (10 pips). ATR_TRAIL_MULT × ATR drives only
               the Phase 2 trailing stop — for EURUSD H1 ATR the ATR term always
               falls below the floor, so the initial SL is always 10 pips.
...
DAILY_ADX_MIN  = 17   # enter range mode when ADX drops below this
DAILY_ADX_HYST = 3    # hysteresis band: trend only resumes at ADX_MIN + ADX_HYST

# Day-of-week gate — blocked weekdays (0=Mon … 4=Fri)
BLOCKED_DAYS: frozenset[int] = frozenset({4})   # Friday: low PF across 60d sample
...
def assess_h1_bias(df: pd.DataFrame, df_4h: Optional[pd.DataFrame] = None,
    ...
    Measure 4 — 4h agreement gate (optional):
        ...
        suppressed as FLAT — these typically correspond to intraday chop
        against the higher-timeframe structure.
